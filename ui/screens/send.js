...
import { parseUnits, elapsed } from '../lib/format.js';
import { markInvalid, markValid } from '../lib/forms.js';
import {
  PHASE_LABELS, CANCELLABLE, ADDRESS_DEBOUNCE_MS, SELF_SEND_QUESTION,
  explainProvingError, isAbortError, stashResult, draftFor, currentSend, startSend,
  plainUnits, checkAmount,
} from './send/state.js';
import {
  shellMarkup, assetStepMarkup, rplOnlyMarkup, detailsStepMarkup,
  reviewStepMarkup, provingStepMarkup, failedStepMarkup,
} from './send/markup.js';
...
registerScreen('send', {
  ...
  async after(ctx, root, arg) {
    ...
    try {
      const [list, info, prove] = await Promise.all([
    ...
    const requested = explicit ? Number(arg) : (assets.length === 1 ? 0 : null);
    ...
    if (requested !== null && requested !== 0) {
      ...
      stepEl.innerHTML = rplOnlyMarkup(assets.find((a) => a.index === requested));
      return;
    }

    const assetIndex = requested === null ? 0 : requested;
    const asset = assets.find((a) => a.index === assetIndex)
      || { index: 0, symbol: 'RAND', decimals: 9, balance: '0', name: 'Rand' };
    const draft = draftFor(ctx, assetIndex);
    ...
    function goStep(next, { focus = true } = {}) {
      ...
      else if (next === 'details') stepEl.innerHTML = detailsStepMarkup(asset, draft, typeof ctx.backend.platform.paste === 'function');
      else if (next === 'review') stepEl.innerHTML = reviewStepMarkup({ asset, to: draft.to, units: reviewUnits, estimate: draft.estimate, canProve });
    ...
    function onStoreChange(store) {
      ...
      stopScreenTicker();
      if (store.error) {
        if (isAbortError(store.error) || store.cancelling) {
      ...
      store.promise.then((result) => {
        if (!live() || !result || !result.hash) return;
        // The send is finished with: clearing it here is what keeps a later visit to #send from
        // bouncing straight back to a receipt whose key has already been handed over.
        ctx.state.send = null;
        stashResult(ctx.session, {
          hash: result.hash,
          txKey: result.txKey || null,
          amount: store.req.amount,
          to: store.req.to,
          assetIndex: store.req.asset,
        });
        ctx.state.sendDraft = null; // this draft has been spent
        ctx.go(`#sent/${result.hash}`);
      });
    }
    ...
    const offMax = on(root, '[data-role="max"]', 'click', async (evt) => {
      ...
      let estimate;
      try {
        estimate = await ctx.backend.send.estimate({
          asset: asset.index,
          // The fee does not depend on who it goes to, so a blank recipient is estimated against
          // the wallet's own address rather than refusing to answer.
          to: (toInput && toInput.value.trim()) || ownAddress,
      ...
      if (!live()) return;
      const fee = BigInt((estimate && estimate.fee) || '0');
    ...
    const offSubmit = on(root, '[data-role="send-form"]', 'submit', async (evt) => {
      ...
      const checked = checkAmount(amountInput.value, asset, draft.estimate ? BigInt(draft.estimate.fee || '0') : null);
