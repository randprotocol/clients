...
/** "1500000000" → "1.5"; trims trailing zeros; optional max fraction digits. */
export function formatUnits(units, maxFrac = 9) {
  const u = toBig(units);
  const neg = u < 0n;
  const a = neg ? -u : u;
  const whole = a / UNITS;
  let frac = (a % UNITS).toString().padStart(9, '0').slice(0, maxFrac).replace(/0+$/, '');
...
export function parseUnits(text) {
  ...
  const [w = '0', f = ''] = s.split('.');
  if (f.length > 9) throw new Error('at most 9 decimal places');
  return BigInt(w || '0') * UNITS + BigInt((f + '000000000').slice(0, 9));
}
