...
/// SHRUGG amounts: 1 SHRUGG = 10^9 units. Mirrors `shrugg_core::{format_amount, parse_amount}`
/// so a UI row never needs a core call.
enum Amount {
    static let unitsPerShrugg: UInt64 = 1_000_000_000
    static let decimals = 9

    static func format(_ units: UInt64) -> String {
        let whole = units / unitsPerShrugg
        let frac = units % unitsPerShrugg
    ...
    /// "1.5" / ".25" / "3" → units. `nil` when it is not a SHRUGG amount.
    static func parse(_ text: String) -> UInt64? {
        ...
        let (mul, o1) = whole.multipliedReportingOverflow(by: unitsPerShrugg)
        if o1 { return nil }
...
extension String {
    /// `shrugg1abcd…wxyz` for addresses, `4f2c…e7` for hashes.
