...
/// JSON-RPC 2.0 over HTTP to a `shrugg-node` (`docs/rpc.md` in the fullnode). One request per
...
final class RpcClient {
    ...
    func sendTransaction(hex: String) async throws -> String {
        guard let h = try await call("shrugg_sendTransaction", [hex]) as? String else {
    ...
    func bridgeEnabled() async throws -> Bool {
        (try await call("shrugg_getBridgeState") as? [String: Any])?["enabled"] as? Bool ?? false
    }

    func blockActions(height: UInt64) async throws -> [Any] {
        let b = try await call("shrugg_getBlockByHeight", [height]) as? [String: Any]
